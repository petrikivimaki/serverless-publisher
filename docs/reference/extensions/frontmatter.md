...
Frontmatter is note-level metadata placed between `---` delimiters at the very beginning of a Markdown file. Papyrus uses it for article identity, search, reader details, header images, and content-rights overrides.
...
All frontmatter values participate in search. The `metadata.allowedKeys` array in `config/app-config.json` separately controls which ordinary properties appear in the reader's Details panel:
...
See [[workflow/setup|Setup workflow]] for the active configuration files and [[reference/extensions/obsidian-syntax|Obsidian syntax and note features]] for links, comments, and math.
