...
The primary menu's tab strip also exposes Bookmarks, Insights, and Links without requiring a permanent tool rail. Its shared active indicator slides between view buttons, while reduced-motion preferences keep the state change immediate. Insights stays intentionally compact: its small uppercase heading follows the same section-title language as **Vault** and **Around Papyrus**, while six centered manifest-derived totals summarize notes, folders, words, estimated reading time, resolved internal links, and unique tags. Four standout-note links follow for the longest, most referenced, most connected, and most recently active articles. “Most connected” counts unique neighboring notes across incoming and outgoing links, while latest activity prefers `updated` frontmatter and falls back to `published`. Ties preserve manifest order. The panel calculates everything from the loaded manifest without downloading article bodies. The Links view is a configurable compact profile with grouped links and local clocks; see [[reference/linktree]] for its card model and automatic service icons. The Library view keeps the vault tree focused on browsing: its **Vault** row reports the note count and provides icon controls to open or close every folder, without duplicating the manifest-backed search field. A subtle, Linktree-style card anchored below the tree identifies the **Local vault** or configured GitHub repository and keeps the content refresh action close to that source context. The card uses the same background as the primary menu and appears only in the Library view, leaving the primary header dedicated to the configured site title and menu close action. The Bookmarks view aligns its empty-state card and bookmarked-note cards to the inset width of the **Remove all bookmarks** button; the empty state uses the same soft card treatment as an active bookmark.

The Search view uses a single **Search notes...** field to match manifest titles, paths, excerpts, and every frontmatter value. Any frontmatter property present in the loaded manifest can scope a query with `property:value`, including `tags:papyrus`, `authors:"Papyrus Team"`, `featured:true`, `published:2026-08-21`, and custom property names. Filters can be combined with ordinary search text or additional property filters. Matching respects metadata shapes: strings use case-insensitive partial matches, arrays test each member, booleans and numbers use typed equality, and explicit null values match `null`. Unrecognized prefixes remain ordinary search text. Search does not download unopened note bodies. Before a query is entered, the search scope and property syntax appear in the same soft card treatment used for the selected result. The first result is selected as the query changes; **Up Arrow** and **Down Arrow** move that selection, and **Enter** opens it while keyboard focus remains in the search field. Search results scroll independently above a compact monthly calendar anchored to the bottom of the view. The calendar follows `appearance.firstDayOfWeek`, marks the current date, includes ISO 8601 week numbers in its first column, and uses its small, unframed month label to return to the current month. Dates with `published` or `updated` frontmatter receive distinct markers and an activity summary. Clicking a highlighted date inserts its plain ISO date, which naturally lists matches from either property and supports multiple notes on the same day.
